...
interface FinancialFunctions {
  ...
  getRecentTransactions: (
    ...
    Array<{ type: string; amount: number; category: string; date: string }>
  >;
}
...
function processFinancialData(expenses: Expense[]) {
  ...
  const monthlyData = expenses.reduce(
    (acc: Record<string, { spent: number; income: number }>, expense) => {
  ...
  contexts.push({
    text: `Financial overview: Total income Rp ${totalIncome.toLocaleString(
      "id-ID"
    )}, total spent Rp ${totalSpent.toLocaleString(
      "id-ID"
  ...
  if (topCategories.length > 0) {
    contexts.push({
      text: `Top spending categories: ${topCategories
  ...
  Object.entries(monthlyData).forEach(([month, data]) => {
    contexts.push({
      text: `${month}: spent Rp ${data.spent.toLocaleString(
        "id-ID"
      )}, income Rp ${data.income.toLocaleString("id-ID")}, net Rp ${(
  ...
  recentTransactions.forEach((expense) => {
    contexts.push({
      text: `${expense.type} on ${expense.date}: Rp ${Number(
        expense.amount
      ).toLocaleString("id-ID")} for ${expense.category}${
...
async function generateAIResponse(
  question: string,
  relevantContext: FinancialContext[],
  allExpenses: Expense[]
): Promise<string> {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: "assistant", content: finalResponse },
          {
            role: "user",
            content:
              "Please enhance this response with any additional insights from the financial context that could be helpful, but keep it short and concise.",
          },
        ],
        temperature: 0,
        max_tokens: 500,
